...
CountSquares[inputPoints_?ListQ] := 
 Module[{count = 0, n = Length[inputPoints], pointA, pointB, pointC, 
  ...
  For[i = 1, i <= n, i++, pointA = inputPoints[[i]];
   For[j = 1, j <= n, j++, pointB = inputPoints[[j]];
    If[i != j, 
     sameDistancePoints = 
      Select[inputPoints, 
       EuclideanDistance[pointA, #] == 
         EuclideanDistance[pointA, pointB] &];
     For[k = 1, k <= Length[sameDistancePoints], k++, 
      pointC = sameDistancePoints[[k]];
      If[VectorAngle[pointB - pointA, pointC - pointA] == Pi/2, 
       pointD = pointB + pointC - pointA;
       If[MemberQ[inputPoints, pointD], count++]]]]]];
  (count/2 )/4(*Since we count each square twice,
